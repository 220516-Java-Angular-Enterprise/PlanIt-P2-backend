...
public interface TripRepository extends CrudRepository<Trips, String> {
    ...
    @Modifying
    @Query(value = "UPDATE trip SET day_activity = ?1, night_activity = ?2, food = ?3 WHERE trip_id= ?4")
    ...
    @Modifying
    @Query(value = "UPDATE trip SET day_activity = ?1 WHERE trip_id= ?2")
    void updateTripDayActivities(String day_activity, String id);

    @Modifying
    @Query(value = "UPDATE trip SET night_activity = ?1 WHERE trip_id= ?2")
    void updateTripNightActivities(String night_activity, String id);

    @Modifying
    @Query(value = "UPDATE trip SET food = ?1 WHERE trip_id= ?2")
    void updateTripFoodActivities(String food, String id);

    @Modifying
    @Query(value = "UPDATE trip SET hotel = ?1 WHERE trip_id= ?2")
